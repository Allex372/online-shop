...
const SingleProduct = ({ data }) => {
  ...
  const { addItemToBacket, items, handleOpenBacket } = backetContext
    ? backetContext
    : {}
  ...
  const selectedCultures =
    cultures?.data?.map(el => el?.attributes?.name.toLowerCase()) || []

  const isAllCultures = CultureArray.every(culture =>
    selectedCultures.includes(culture.name.toLowerCase())
  )
      ...
      <Layout>
        ...
        <div className={styles.wrapper}>
          ...
          <div className={styles.infoWrapper}>
            <div className={styles.imgWrapper}>
              <div className={styles.titleWrapper}>
                ...
                {!isAvailable && (
                  <p className={styles.notAvailable}>
                    Під замовлення (до 3-ох робочих днів)
                  </p>
                )}
              </div>
              ...
              {isInBacket ? (
                <button
                  className={styles.buyButtonAdded}
                ...
                <button
                  ...
                  onClick={() => handleAddItem(productData, id)}
                >
                  Купити
                </button>
              )}
        ...
        <ImageModal isOpen={openImageModal} data={Table?.data?.attributes?.url}>
          <button
            ...
            onKeyDown={e =>
              (e.key === "Enter" || e.key === " ") && setOpenImageModal(false)
            }
          >
